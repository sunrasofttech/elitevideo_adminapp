    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        if (result['status'] == true) {
          ...
          emit(UpdateFilmErrorState(result['message'].toString()));
        }
      } else {
        emit(UpdateFilmErrorState("${result['message'] ?? "Server Error"}"));
      }
    } catch (e) {
      log("Error: $e");
