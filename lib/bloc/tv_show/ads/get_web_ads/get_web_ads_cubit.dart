...
class GetTvShowWebAdsCubit extends Cubit<GetTvShowWebAdsState> {
  GetTvShowWebAdsCubit() : super(GetWebAdsInitial());

  Future<void> getAllWebAds({int page = 1, int limit = 10}) async {
    try {
      emit(GetWebAdsLoadingState());

      final uri = Uri.parse("${AppUrls.seasonAdsUrl}/getall")
          .replace(queryParameters: {'page': page.toString(), 'limit': limit.toString(), "show_type": "tvshows"});
